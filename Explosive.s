...
first_attack: sti r10, r15, r9
	live %42
	zjmp %-410
p2: ld %50986512, r10		# 02 90 03 09 fe 10 0a
	ld %-392, r9
	...
	ld -511, r1
	zjmp %:first_attack
begin: ld %55575553, r8		# 02 90 03 50 04 01 08
	st r1, r15				# 03 50 01 0f
	st r8, -4				# 03 70 08 ff fc
	fork %:def
	live %42
	fork %:p2
p1: ld %190054913, r10		# 02 90 0b 54 02 01 0a
	ld %-396, r9
...
store_def: st r4, -37
	...
	live %42
	zjmp %:store_def
def: ld %252645135, r4		# 02 90 0f 0f 0f 0f 04
	live %42
	ld 511, r1
	fork %:second_attack
	st r15, -27
